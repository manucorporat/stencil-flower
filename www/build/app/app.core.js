...
(function(window, document, Context, namespace) {
  ...
  function setAccessor(plt, elm, memberName, oldValue, newValue, isSvg, isHostElement) {
    if ('class' !== memberName || isSvg) {
      if ('style' === memberName) {
        for (const prop in oldValue) {
          newValue && null != newValue[prop] || elm.style.removeProperty(prop);
        }
        for (const prop in newValue) {
          oldValue && newValue[prop] === oldValue[prop] || elm.style.setProperty(prop, newValue[prop]);
        }
    ...
    if (oldValue !== newValue) {
      ...
      ...newList.filter(item => !oldList.includes(item)) ].join(' ');
    }
  }
